import { ActionPanel, Action, Form, Toast, showToast, Icon, List, useNavigation, Color } from "@raycast/api";
import { useState, useEffect } from "react";
import fs from "fs";
import path from "path";

const FAST_FLAGS_PATH = "/Applications/Roblox.app/Contents/MacOS/ClientSettings/ClientAppSettings.json";
...
export default function Command() {
  ...
  useEffect(() => {
    if (searchText) {
      setFilteredFlags(flags.filter((flag) => flag.name.toLowerCase().includes(searchText.toLowerCase())));
    } else {
  ...
  async function loadFastFlags() {
    ...
    try {
      if (!fs.existsSync(FAST_FLAGS_PATH)) {
        throw new Error(`Roblox fast flags file not found at: ${FAST_FLAGS_PATH}`);
      }
      ...
      searchBarAccessory={
        ...
        >
          <List.Dropdown.Section title="Actions">
            <List.Dropdown.Item title="Select Action..." icon={Icon.Dot} value="" />
            <List.Dropdown.Item title="Add New Flag" icon={Icon.Plus} value="add" />
            <List.Dropdown.Item title="Refresh Flags" icon={Icon.RotateClockwise} value="refresh" />
    ...
    >
      {error ? (
        <List.EmptyView
          ...
          actions={
            <ActionPanel>
              <Action title="Retry" onAction={loadFastFlags} icon={Icon.RotateClockwise} />
              <Action
                title="Add New Flag"
                icon={Icon.Plus}
                onAction={() => push(<AddFlagScreen onFlagAdded={loadFastFlags} />)}
              />
            </ActionPanel>
          }
        />
              ...
              <ActionPanel>
                <ActionPanel.Section title="Flag Actions">
                  <Action
                    title="Edit Flag"
                    icon={Icon.Pencil}
                    onAction={() => push(<EditFlagScreen flag={flag} onFlagUpdated={loadFastFlags} />)}
                  />
                  <Action
                    title="Delete Flag"
...
async function handleDeleteFlag(flagName: string, onDeleted: () => void) {
  ...
  try {
    ...
    const backupPath = path.join(path.dirname(FAST_FLAGS_PATH), "ClientAppSettings.backup.json");
...
function AddFlagScreen({ onFlagAdded }: { onFlagAdded: () => void }) {
  ...
  const [errorMessages, setErrorMessages] = useState<{ [key: string]: string | undefined }>({});
  ...
  return (
    <Form
      actions={
        <ActionPanel>
          <Action.SubmitForm title="Add Flag" icon={Icon.Plus} onSubmit={handleSubmit} />
...
function EditFlagScreen({ flag, onFlagUpdated }: { flag: FastFlag; onFlagUpdated: () => void }) {
  const { pop } = useNavigation();
  const [errorMessages, setErrorMessages] = useState<{ [key: string]: string | undefined }>({});

  async function handleSubmit(values: Record<string, string | boolean | number>) {
    const errors: { [key: string]: string } = {};
  ...
  return (
    <Form
      actions={
        <ActionPanel>
          <Action.SubmitForm title="Save Changes" icon={Icon.SaveDocument} onSubmit={handleSubmit} />
          <Action title="Cancel" icon={Icon.XmarkCircle} onAction={pop} />
        </ActionPanel>
      }
    >
      ...
      {flag.type === "boolean" ? (
        <Form.Dropdown id="value" title="Value" defaultValue={String(flag.value)}>
        ...
        <Form.TextField id="value" title="Value" placeholder="Enter string value" defaultValue={String(flag.value)} />
      )}
    </Form>
  );
}
